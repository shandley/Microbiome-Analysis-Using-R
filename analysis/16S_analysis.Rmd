---
title: "Using R to Analyze the Bacterial Microbiome Workshop"
...
4) Sample summary and assessment
...
ps0 <- readRDS("~/workshop_materials/Microbiome/data/16S_data.RDS")
ps0

# Read in a mapping file containing sample variable information
map <- import_qiime_sample_data("~/workshop_materials/Microbiome/data/16S_mapping.txt")
dim(map)
...
With multiplexing technology it is normal to sequence large numbers of samples at once. PhyloSeq makes easy work of separating out and subsetting irrelevant from relevant samples.
...
p.smoothed.phylum <- ggplot(dat.1, aes(x = treatment_days, y = Abundance, color = Phylum, group = Phylum)) +
...
p.diffab <- ggplot(df.rdp, aes(x = Phylum, y = log2FoldChange, color = Family)) +
  geom_jitter(size = 3, alpha = 0.7, width = 0.1) +
  geom_hline(yintercept = 0, lty = 2) +
  ylim(-35, 35)
p.diffab

# The taxa on the bottom are more abundant in the control mice
# The taxa on the top are more abundant in the antibiotics treated mice
